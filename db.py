...
class Database:
    ...
    def _migrate_orders_table(self, conn: sqlite3.Connection) -> None:
        ...
        if "catalog_item_id" not in columns:
            cur.execute("ALTER TABLE orders ADD COLUMN catalog_item_id INTEGER")
            conn.commit()
    ...
    def create_order(self, data: dict[str, Any]) -> int:
        ...
        cur.execute(
            """
            INSERT INTO orders (
                user_id, catalog_item_id, user_name, user_phone, equipment_type, attachments, hours, date, status
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
    ...
    def list_active_orders_for_item(self, catalog_item_id: int) -> list[dict[str, Any]]:
        conn = self._connect()
        cur = conn.cursor()
        cur.execute(
            """
    ...
    def update_catalog_item(self, item_id: int, data: dict[str, Any]) -> bool:
        allowed = {
            "item_type",
            "title",
            "description",
        ...
        for field in fields:
            value = data[field]
            if field in {"attachments_enabled", "is_active"}:
        ...
        cur = conn.cursor()
        cur.execute(
            f"UPDATE catalog_items SET {', '.join(assignments)} WHERE id = ?",
            values,
        )
